...
// TokenRevokeKycTransaction revokes KYC status from an account for a token
pub const TokenRevokeKycTransaction = struct {
    base: Transaction,
    token_id: ?TokenId,
    account_id: ?AccountId,
    
    pub fn init(allocator: std.mem.Allocator) TokenRevokeKycTransaction {
    ...
    pub fn deinit(self: *TokenRevokeKycTransaction) void {
        self.base.deinit();
    }
    
    // Set the token to revoke KYC for
    pub fn setTokenId(self: *TokenRevokeKycTransaction, token_id: TokenId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        self.token_id = token_id;
    }
    
    // Set the account to revoke KYC from
    pub fn setAccountId(self: *TokenRevokeKycTransaction, account_id: AccountId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        self.account_id = account_id;
    }
    
    // Execute the transaction
    pub fn execute(self: *TokenRevokeKycTransaction, client: *Client) !TransactionResponse {
    ...
    pub fn buildTransactionBody(self: *TokenRevokeKycTransaction) ![]u8 {
        ...
        try writer.writeMessage(34, kyc_bytes);
        
        return writer.toOwnedSlice();
    }
    
    fn writeCommonFields(self: *TokenRevokeKycTransaction, writer: *ProtoWriter) !void {
        // transactionID = 1
