...
pub const TokenAssociateTransaction = struct {
    ...
    pub fn setAccountId(self: *TokenAssociateTransaction, account_id: AccountId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        self.account_id = account_id;
    }
    
    // Includes a token for association
    pub fn addTokenId(self: *TokenAssociateTransaction, token_id: TokenId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        
        if (self.token_ids.items.len >= MAX_TOKEN_ASSOCIATIONS) {
            return error.TooManyTokenAssociations;
        }
    ...
    pub fn setTokenIds(self: *TokenAssociateTransaction, token_ids: []const TokenId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
    ...
    pub fn execute(self: *TokenAssociateTransaction, client: *Client) !TransactionResponse {
        if (self.account_id == null) {
            return error.AccountIdRequired;
        }
        
        if (self.token_ids.items.len == 0) {
            return error.NoTokensToAssociate;
        }
    ...
    pub fn buildTransactionBody(self: *TokenAssociateTransaction) ![]u8 {
        ...
        // tokenAssociate = 40 (oneof data)
        var associate_writer = ProtoWriter.init(self.base.allocator);
        defer associate_writer.deinit();
        ...
        if (self.account_id) |account| {
            ...
            try associate_writer.writeMessage(1, account_bytes);
        }
        ...
        const associate_bytes = try associate_writer.toOwnedSlice();
        defer self.base.allocator.free(associate_bytes);
        try writer.writeMessage(40, associate_bytes);
        
        return writer.toOwnedSlice();
    }
    
    fn writeCommonFields(self: *TokenAssociateTransaction, writer: *ProtoWriter) !void {
        // transactionID = 1
