...
pub const TokenWipeTransaction = struct {
    ...
    pub fn setTokenId(self: *TokenWipeTransaction, token_id: TokenId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        self.token_id = token_id;
    }
    
    // Set the account to wipe from
    pub fn setAccountId(self: *TokenWipeTransaction, account_id: AccountId) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        self.account_id = account_id;
    }
    
    // Set amount to wipe (for fungible tokens)
    pub fn setAmount(self: *TokenWipeTransaction, amount: u64) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
        ...
        if (amount == 0) {
            return error.InvalidWipeAmount;
        }
        
        if (amount > std.math.maxInt(i64)) {
            return error.WipeAmountTooLarge;
        }
    ...
    pub fn addSerialNumber(self: *TokenWipeTransaction, serial_number: i64) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
    ...
    pub fn setSerialNumbers(self: *TokenWipeTransaction, serial_numbers: []const i64) !void {
        if (self.base.frozen) return error.TransactionIsFrozen;
    ...
    pub fn addSerial(self: *TokenWipeTransaction, serial: i64) !void {
