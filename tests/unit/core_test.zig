...
test "AccountId.fromProtobuf parses valid protobuf" {
    const allocator = testing.allocator;
    const ProtoWriter = hedera.ProtoWriter;
    ...
    // Parse it back
    const account_id = try hedera.AccountId.fromProtobuf(allocator, protobuf_data);
...
test "AccountId.fromProtobuf roundtrip with toProtobuf" {
    ...
    // Create an AccountId
    const original = hedera.AccountId.init(7, 8, 9999);
    ...
    // Parse it back
    const parsed = try hedera.AccountId.fromProtobuf(allocator, protobuf_data);
...
test "TokenId.fromProtobuf parses valid protobuf" {
    const allocator = testing.allocator;
    const ProtoWriter = hedera.ProtoWriter;
    
    // Create protobuf data for TokenId with shard=0, realm=0, num=5555
    var writer = ProtoWriter.init(allocator);
    defer writer.deinit();
    
    try writer.writeInt64(1, 0); // shard
    try writer.writeInt64(2, 0); // realm  
    try writer.writeInt64(3, 5555); // token num
    
    const protobuf_data = try writer.toOwnedSlice();
    defer allocator.free(protobuf_data);
    
    // Parse it back
    const token_id = try hedera.TokenId.fromProtobuf(allocator, protobuf_data);
    
    try testing.expectEqual(@as(u64, 0), token_id.shard());
    try testing.expectEqual(@as(u64, 0), token_id.realm());
    try testing.expectEqual(@as(u64, 5555), token_id.num());
}

test "TokenId.fromProtobuf roundtrip with toProtobuf" {
    const allocator = testing.allocator;
    
    // Create a TokenId
    const original = hedera.TokenId.init(2, 3, 7777);
    ...
    // Parse it back
    const parsed = try hedera.TokenId.fromProtobuf(allocator, protobuf_data);
    
    // Should be identical
    try testing.expect(original.equals(parsed));
    try testing.expectEqual(original.shard(), parsed.shard());
    try testing.expectEqual(original.realm(), parsed.realm());
    try testing.expectEqual(original.num(), parsed.num());
}

test "ContractId.fromProtobuf roundtrip" {
    const allocator = testing.allocator;
    
    const original = hedera.ContractId.init(1, 2, 3333);
    const protobuf_data = try original.toProtobuf(allocator);
    defer allocator.free(protobuf_data);
    
    const parsed = try hedera.ContractId.fromProtobuf(allocator, protobuf_data);
...
test "FileId.fromProtobuf roundtrip" {
    const allocator = testing.allocator;
    
    const original = hedera.FileId.init(0, 0, 111);
    const protobuf_data = try original.toProtobuf(allocator);
    defer allocator.free(protobuf_data);
    
    const parsed = try hedera.FileId.fromProtobuf(allocator, protobuf_data);
...
test "TopicId.fromProtobuf roundtrip" {
    const allocator = testing.allocator;
    
    const original = hedera.TopicId.init(0, 0, 4444);
    const protobuf_data = try original.toProtobuf(allocator);
    defer allocator.free(protobuf_data);
    
    const parsed = try hedera.TopicId.fromProtobuf(allocator, protobuf_data);
...
test "ScheduleId.fromProtobuf roundtrip" {
    const allocator = testing.allocator;
    
    const original = hedera.ScheduleId.init(0, 0, 555);
    const protobuf_data = try original.toProtobuf(allocator);
    defer allocator.free(protobuf_data);
    
    const parsed = try hedera.ScheduleId.fromProtobuf(allocator, protobuf_data);
