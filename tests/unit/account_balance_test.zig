...
test "AccountBalance token balances" {
    ...
    try balance.setTokenBalance(token1, 1000);
    try balance.setTokenBalance(token2, 2000);
...
test "AccountBalance decimal balances" {
    ...
    try balance.setTokenBalanceWithDecimals(token, 1000, 8);
...
test "AccountBalance serialization" {
    ...
    try balance.setTokenBalance(TokenId.init(0, 0, 600), 3000);
    ...
    try testing.expectEqual(balance.account_id.?.num, deserialized.account_id.?.num);
